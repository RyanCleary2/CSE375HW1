...
//Not sure if this is a problem since you said we implement using primitive data types 
//but this is what my partner and I did for 303
struct Bank {
...
void create_map(Bank &bank) {
	...
	double balance = 100000.0 / num_accounts;
	...
	cout << "Map created with : " << total << endl;
}
...
void deposit(Bank &bank) {
	...
	std::uniform_int_distribution<int> account(0, 99);
	std::uniform_int_distribution<int> transfer(0, 10000);
	...
	//Always lock the account with the lower index first, to reduce deadlocks
	if (account1 != account2) {
		if (account1 > account2) {
			std::swap(account1, account2);
		}
		//Lock both accounts at the same time to avoid deadlocks
		std::lock(bank.accounts[account1].lock, bank.accounts[account2].lock);
		//Use lockguards to take ownership of the locks and automatically unlock them when the function ends
		std::lock_guard<std::mutex> lock1(bank.accounts[account1].lock, std::adopt_lock);
		std::lock_guard<std::mutex> lock2(bank.accounts[account2].lock, std::adopt_lock);
...
float balance(Bank &bank) {
	//Lock the entire map and release when function ends
	float total = 0;
	for (int i = 0; i < 100; i++) {
		std::lock_guard<std::mutex> guard(bank.accounts[i].lock);
		total += bank.accounts[i].balance;
	}
	return total;
}
...
//Same as the global lock version
